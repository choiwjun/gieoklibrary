...
## [] M2.1: Supabase Auth 설정 및 회원가입 구현
...
- [ ]  이메일과 비밀번호로 회원가입 가능
- [ ]  비밀번호 확인 검증이 작동함
- [ ]  회원가입 시 user_profiles 테이블에 레코드 생성됨
- [ ]  에러 메시지가 명확하게 표시됨
- [ ]  로딩 상태가 표시됨
- [ ]  회원가입 후 이메일 확인 페이지로 이동
...
- [ ]  로그인 후 메인 페이지로 이동
- [ ]  last_login_at이 업데이트됨
- [ ]  로딩 상태 표시
- [ ]  비밀번호 찾기 링크 작동

### 자가 수정 지침

- [ ]  autoComplete 속성이 올바르게 설정되었는가?
- [ ]  로그인 실패 시 보안을 위해 구체적인 정보를 노출하지 않는가?
- [ ]  세션이 올바르게 생성되는가?
...
- [ ]  서버 컴포넌트와 클라이언트 컴포넌트가 올바르게 분리되었는가?
- [ ]  폼 검증이 충분한가?
- [ ]  에러 핸들링이 적절한가?
